...
public class RenderGlobal implements IWorldAccess
{
    ...
    public void renderEntities(EntityLivingBase p_147589_1_, ICamera p_147589_2_, float p_147589_3_)
    {
        ...
        else
        {
            ...
            List var31 = this.theWorld.getLoadedEntityList();

            if (pass == 0)
            {
                this.countEntitiesTotal = var31.size();
            }
            ...
            for (var18 = 0; var18 < var31.size(); ++var18)
            {
                var19 = (Entity)var31.get(var18);
            ...
            {
                TileEntity var32 = (TileEntity)this.tileEntities.get(var18);

                if (!hasTileEntityShouldRenderInPass || Reflector.callBoolean(var32, Reflector.ForgeTileEntity_shouldRenderInPass, new Object[] {Integer.valueOf(pass)}))
                {
                    AxisAlignedBB var33 = this.getTileEntityBoundingBox(var32);

                    if (var33 == AABB_INFINITE || p_147589_2_.isBoundingBoxInFrustum(var33))
                    {
                        Class var34 = var32.getClass();

                        if (var34 == TileEntitySign.class && !Config.zoomMode)
                        {
                            EntityClientPlayerMP block = this.mc.thePlayer;
                            double distSq = var32.getDistanceFrom(block.posX, block.posY, block.posZ);
                            ...
                            if (distSq > 256.0D)
                            {
                                ...
                                if (Config.isShaders())
                                {
                                    Shaders.nextBlockEntity(var32);
                                }

                                TileEntityRendererDispatcher.instance.renderTileEntity(var32, p_147589_3_);
                        ...
                        if (var34 == TileEntityChest.class)
                        {
                            Block var35 = this.theWorld.getBlock(var32.xCoord, var32.yCoord, var32.zCoord);

                            if (!(var35 instanceof BlockChest))
                            {
                                continue;
                            }
                        }
    ...
    public void renderSky(float par1)
    {
        ...
        else if (this.mc.theWorld.provider.isSurfaceWorld())
        {
            ...
            if (var27 != null && Config.isSunMoonEnabled())
            {
                ...
                {
                    var20 = (float)var30 * (float)Math.PI * 2.0F / (float)var29;
            ...
            if (this.mc.theWorld.provider.isSurfaceWorld())
            {
                ...
                else
                {
                    ...
                    if (this.isFancyGlListClouds || this.cloudTickCounter >= this.cloudTickCounterGlList + 20)
                    {
                        ...
                        float var211 = (float)(var15 * (double)var10);
